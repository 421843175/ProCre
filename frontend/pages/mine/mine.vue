<template>
	<view>
		<div class="mine">
			<view class="people" style="margin-top:50rpx;">
				<image src="../../static/未登录.jpeg" class="peopleimg"></image>

				<span class="hipo" style="margin-top:30rpx;">{{timehello}},
				<span @click="islogin">{{userhello}}</span>
				</span>
			</view>
			<view class="peopleabout"  style="margin-top:50rpx;">
				<span><p class="num">0</p>帖子</span>
				<span><p class="num">0</p>评论</span>
				<span><p class="num">0</p>关注</span>
				<span><p class="num">0</p>粉丝</span>
				
			</view>
			<view>
				<!-- :border="false" 上下隐藏边框 -->
				<uni-list class="mplist" :border="false">
					<!-- showExtraIcon 左侧是否显示扩展图标 -->
				<uni-list-item title="浏览历史" showExtraIcon @tap="openHistory">
					<text slot="icon">
						<svg t="1686125544414" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2643" width="16px" height="16px"><path d="M512 938.666667c235.648 0 426.666667-191.018667 426.666667-426.666667S747.648 85.333333 512 85.333333 85.333333 276.352 85.333333 512s191.018667 426.666667 426.666667 426.666667z m0 85.333333C229.248 1024 0 794.752 0 512S229.248 0 512 0s512 229.248 512 512-229.248 512-512 512z m42.666667-486.869333V298.538667C554.666667 275.328 535.552 256 512 256c-23.722667 0-42.666667 19.029333-42.666667 42.538667v256.256a41.984 41.984 0 0 0 12.202667 29.866666l121.258667 121.258667a42.368 42.368 0 0 0 60.032-0.298667 42.666667 42.666667 0 0 0 0.298666-60.032L554.666667 537.130667z" fill="#3D3D3D" p-id="2644"></path></svg>
					</text>
					
				</uni-list-item>
				<uni-list-item title="我的收藏" showExtraIcon @tap="openHistory">
					<text slot="icon">
<svg t="1686143221248" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="11819" width="16" height="16"><path d="M512 720.416l232.096 171.904a19.2 19.2 0 0 0 16.864 2.88c9.536-2.88 14.592-12.224 11.84-20.864l-88.96-281.6 237.344-176.96a16.48 16.48 0 0 0 6.816-13.152c0-9.152-7.904-16.928-18.112-16.96l-292-0.448-88.48-277.12a17.568 17.568 0 0 0-11.776-11.264c-9.856-2.976-20.16 2.304-23.04 11.264l-88.48 277.12-292 0.48a18.624 18.624 0 0 0-14.624 6.944 16.096 16.096 0 0 0 3.328 23.136l237.376 176.96-88.992 281.6a15.904 15.904 0 0 0 2.72 14.688c6.048 7.744 17.856 9.312 25.984 3.296L512 720.416z m-194.016 223.36a83.008 83.008 0 0 1-114.56-15.424 79.904 79.904 0 0 1-13.28-73.28l75.296-238.24-200.864-149.76a80.096 80.096 0 0 1-15.424-113.92 82.624 82.624 0 0 1 64.864-31.456l245.312-0.384 74.304-232.672c13.6-42.56 59.52-66.112 102.56-53.024A81.536 81.536 0 0 1 590.4 88.64l74.304 232.64 245.312 0.416c45.184 0.064 82.08 36.16 82.016 81.024a80.48 80.48 0 0 1-32.576 64.352l-200.864 149.76 75.296 238.24c13.568 42.976-11.072 88.352-54.496 101.408a83.2 83.2 0 0 1-73.344-12.736L512 800.064l-194.016 143.68z" fill="#2c2c2c" p-id="11820"></path></svg>					
					
					</text>
					
				</uni-list-item>
				<uni-list-item title="我的创造粒" showExtraIcon @tap="openIntegral">
					<text slot="icon">
<svg t="1686127566815" class="icon" viewBox="0 0 1025 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4799" width="16" height="16"><path d="M874.032101 149.9439c-199.930533-199.930533-524.113669-199.930533-724.076201 0.031999s-199.930533 524.08167 0.031999 724.012203c199.962532 199.962532 524.113669 199.962532 724.044202 0.063998s199.962532-524.145668 0-724.1082zM628.31082 640.074498c-51.93458 51.93458-93.085455 106.205096-119.708727 154.747769-26.623272-48.542673-67.774147-102.813189-119.708727-154.747769s-106.205096-93.085455-154.747769-119.708727c48.542673-26.623272 102.813189-67.774147 154.747769-119.708727s93.085455-106.205096 119.708727-154.747769c26.623272 48.542673 67.774147 102.813189 119.708727 154.747769s106.205096 93.085455 154.747769 119.708727c-48.510674 26.623272-102.813189 67.806146-154.747769 119.708727z" fill="#898989" p-id="4800"></path></svg>
					</text>
					
				</uni-list-item>
				<uni-list-item title="我的成就" showExtraIcon @tap="openHistory">
									<text slot="icon">
<svg t="1686143672202" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2685" width="16" height="16"><path d="M934.679128 70.180685h-66.480249V58.824174c0-30.7519-24.882243-55.634143-55.634144-55.634143H209.266044c-30.6881 0-55.634143 24.882243-55.634144 55.634143V70.180685H82.94081C41.598006 70.180685 7.975078 103.803614 7.975078 145.146417v150.697072C7.975078 368.384798 67.054455 427.464174 139.595763 427.464174h15.822555c16.141558 165.754019 145.529221 298.204112 309.815825 319.194518v168.880249H250.353645c-53.401121 0-96.721745 43.320623-96.721745 96.721744v11.739315h714.566979v-11.739315c0-53.401121-43.256822-96.721745-96.657944-96.721744H556.660436v-168.880249c164.286604-20.990405 293.674268-153.440498 309.815826-319.194518h11.547912C950.565483 427.464174 1009.64486 368.384798 1009.64486 295.843489V145.146417c0-41.342804-33.622928-74.965732-74.965732-74.965732zM153.6319 363.663551h-14.036137C102.208598 363.663551 71.775701 333.230654 71.775701 295.843489V145.146417c0-6.12486 5.040249-11.165109 11.165109-11.165109h70.69109v229.682243z m521.314891-30.815701L515.828037 491.966604c-3.891838 3.891838-10.144299 3.891838-14.036137 0L342.673146 332.84785c-2.998629-2.998629-3.764237-7.592274-1.914018-11.420311l42.363613-86.896449c1.658816-3.381433 5.16785-5.550654 8.932088-5.550654h233.51028c3.764237 0 7.273271 2.169221 8.932087 5.550654l42.363614 86.896449c1.850218 3.828037 1.084611 8.421682-1.914019 11.420311z m270.897446-37.004361c0 37.387165-30.432897 67.820062-67.820063 67.820062h-9.825295V133.981308h66.480249c6.12486 0 11.165109 5.040249 11.165109 11.165109v150.697072z" fill="#2c2c2c" p-id="2686"></path></svg>

									</text>
									
								</uni-list-item>
				<uni-list-item title="用户体验调研" showExtraIcon @tap="openHistory">
				<text slot="icon">
<svg t="1686143972359" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4016" width="16" height="16"><path d="M847.5 235.7H180.8c-4.7 0-8.6-3.8-8.6-8.6V188c0-4.7 3.8-8.6 8.6-8.6h666.7c4.7 0 8.6 3.8 8.6 8.6v39.1c0 4.7-3.8 8.6-8.6 8.6z m-661.3-14h655.9v-28.3H186.2v28.3zM847.5 865.8H180.8c-4.7 0-8.6-3.8-8.6-8.6v-39.1c0-4.7 3.8-8.6 8.6-8.6h666.7c4.7 0 8.6 3.8 8.6 8.6v39.1c0 4.8-3.8 8.6-8.6 8.6z m-661.3-14h655.9v-28.3H186.2v28.3z" fill="#2c2c2c" p-id="4017"></path><path d="M192.9 228.7h14v587.9h-14zM812.5 228.7h14v587.9h-14z" fill="#2c2c2c" p-id="4018"></path><path d="M269.5 277.3h489.3V497H269.5z" fill="#2c2c2c" p-id="4019"></path><path d="M329.1 474.3c-13.7 0-29.4-2.1-47-6.2l3.2-13.6c41 9.6 69.5 7.5 84.9-6.3 10-9 13.6-22.5 16.5-33.4 3.1-11.5 6-22.4 16.4-23.9 10.3-1.4 15.8 9.6 21.1 20.2 2 4.1 6 12.2 8.6 14.2 2.3-3 6.3-12.4 10.9-23.1 2.8-6.6 6.4-14.9 10.7-24.7 9.8-22.1 15.7-31.6 26.4-33.9 17.8-3.7 26.2 11.1 31.8 20.9 3.6 6.3 6.3 10.7 8.3 10.1 12.1-3.5 22.9-0.4 32 9.1 7.1 7.5 12.3 17.8 17.4 27.9 5.6 11.1 11.4 22.7 17.7 24.8 1.3 0.4 3.6 0.8 7.5-1.2 7.8-3.9 11.3-24.9 14.4-43.4 4.9-29.1 10.4-62.1 35.5-67 12.9-2.5 23.9 2.4 32.8 14.8 6.9 9.5 11.6 22.1 15.8 33.2 2.7 7.3 7.3 19.4 10.2 21.3 1.8-0.2 13.1-4.2 33.9-66.6l13.3 4.4c-9.3 27.9-17.6 47.4-25.6 59.7-7.5 11.6-15 17-22.9 16.4-10.9-0.8-15.7-13.9-21.9-30.4-9-24-17.2-42.2-32.8-39.1-15.6 3.1-20.3 31-24.4 55.6-4.2 24.9-7.8 46.4-21.9 53.5-6.4 3.2-12.5 3.9-18.2 2-11.9-3.9-18.6-17.5-25.8-31.8-10.7-21.4-18.3-34-32.9-29.8-12.8 3.7-19.5-8-24.4-16.6-6.1-10.7-9.8-15.6-16.8-14.1-4.1 0.8-8.2 7.4-16.5 25.9-4.3 9.7-7.8 17.9-10.7 24.5-4.6 10.7-7.6 17.7-10.3 22.4-1.6 2.9-5.5 9.6-12.9 9.6h-0.2c-10.5-0.2-16.1-11.5-21.5-22.4-1.9-3.8-4.9-9.9-7-12.1-1.5 2.2-3.3 8.7-4.5 13.1-3.2 12-7.6 28.4-20.7 40.2-11.7 10.5-28.5 15.8-50.4 15.8z" fill="#2c2c2c" p-id="4020"></path><path d="M289.8 552.5h71.6v14h-71.6zM386.9 552.5h125.9v14H386.9zM479.5 601.4h25.1v14h-25.1zM289.8 601.4h171.9v14H289.8zM289.8 648.6h102v14h-102zM412.9 648.6h99.9v14h-99.9zM386.9 700h125.9v14H386.9zM339 699.3h25.1v14H339zM287.1 699.3h25.1v14h-25.1z" fill="#2c2c2c" p-id="4021"></path><path d="M750.4 608.5c-7.9-24.9-26.5-45-50.4-55l-34.2 82 84.6-27z" fill="#2c2c2c" p-id="4022"></path><path d="M660.1 735.3c-52.8 0-95.8-43-95.8-95.8s43-95.8 95.8-95.8c12.7 0 25.2 2.5 36.9 7.4l6.5 2.7-31.1 74.5 76.9-24.5 2.1 6.7c3 9.5 4.5 19.2 4.5 29.1 0 52.7-43 95.7-95.8 95.7z m0-177.6c-45.1 0-81.8 36.7-81.8 81.8 0 45.1 36.7 81.8 81.8 81.8 45.1 0 81.8-36.7 81.8-81.8 0-6.1-0.7-12.2-2-18.1l-92.1 29.4 37.2-89.2c-8-2.6-16.4-3.9-24.9-3.9z" fill="#2c2c2c" p-id="4023"></path></svg>								
			</text>
																	
				</uni-list-item>
				<uni-list-item title="审核中心" showExtraIcon @tap="openHistory">
					<text slot="icon">
						<svg t="1686127476429" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3690" width="16" height="16"><path d="M426.496 707.072c-10.24 0-20.992-4.096-29.184-11.776l-138.24-138.24c-15.872-15.872-15.872-41.984 0-57.856 15.872-15.872 41.984-15.872 57.856 0l109.056 109.056L706.56 328.192c15.872-15.872 41.984-15.872 57.856 0 15.872 15.872 15.872 41.984 0 57.856L455.68 694.784c-8.192 8.192-18.944 12.288-29.184 12.288z" fill="#333333" p-id="3691"></path><path d="M840.192 983.552H182.784c-78.848 0-143.36-64.512-143.36-143.36V182.784c0-78.848 64.512-143.36 143.36-143.36h657.408c36.352 0 71.168 13.824 97.792 38.4 29.184 27.136 45.568 65.024 45.568 104.96v449.024c0 22.528-18.432 40.96-40.96 40.96s-40.96-18.432-40.96-40.96V182.784c0-17.408-7.168-33.28-19.456-45.056-11.264-10.752-26.112-16.384-41.984-16.384H182.784c-33.792 0-61.44 27.648-61.44 61.44v657.408c0 33.792 27.648 61.44 61.44 61.44h657.408c33.792 0 61.44-27.648 61.44-61.44 0-22.528 18.432-40.96 40.96-40.96s40.96 18.432 40.96 40.96c0 79.36-64 143.36-143.36 143.36z" fill="#333333" p-id="3692"></path></svg>
						
					</text>
					
				</uni-list-item>
				<uni-list-item title="管理中心" showExtraIcon @tap="openHistory">
					<text slot="icon">
						<svg t="1686129057917" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5886" width="16" height="16"><path d="M878.592 781.312q4.096 14.336 7.168 36.352t3.072 44.544-3.584 42.496-10.752 30.208q-5.12 7.168-26.112 13.824t-52.736 12.288-70.656 10.752-77.824 8.704-75.776 5.12-64.512 1.536-64-2.048-74.752-5.12-76.288-7.168-69.12-9.216-53.248-9.728-27.136-9.728q-9.216-7.168-14.336-45.568t4.096-99.84q5.12-34.816 28.16-53.248t53.248-29.184 63.488-18.944 57.856-24.576q19.456-12.288 30.208-23.552t15.36-22.528 4.608-23.552-1.024-27.648q-2.048-22.528-14.848-34.816t-28.16-25.6q-8.192-6.144-13.824-17.92t-10.752-24.064q-5.12-13.312-9.216-29.696-2.048-2.048-6.144-7.168-3.072-5.12-7.168-13.824t-9.216-25.088-6.656-29.184 0.512-22.016q1.024-11.264 5.12-19.456 0-35.84 4.096-71.68 4.096-29.696 13.312-64.512t29.696-62.464q18.432-25.6 39.936-42.496t44.544-26.112 46.592-12.8 45.056-3.584q23.552 2.048 38.4 5.632t24.064 8.704 15.872 12.288 13.824 14.336l19.456 0q9.216 0 17.92 2.56t17.408 9.728 18.944 20.48q23.552 30.72 34.816 67.072t16.384 69.12q5.12 37.888 6.144 74.752 2.048 5.12 3.072 12.288t1.536 17.408-1.536 25.6q-2.048 20.48-6.144 32.256t-9.216 17.92q-5.12 7.168-11.264 10.24-4.096 16.384-9.216 29.696-5.12 12.288-10.752 24.064t-13.824 17.92q-17.408 14.336-28.672 24.064t-15.36 33.28q-3.072 14.336-2.048 28.672t7.68 29.184 21.504 28.16 41.472 23.552q23.552 9.216 51.712 16.896t54.272 17.408 46.592 25.088 28.672 39.936zM549.888 747.52q3.072-6.144 0.512-12.8t-7.68-12.8-12.288-13.312l-47.104 0q-7.168 7.168-11.264 13.312-4.096 5.12-6.656 11.264t0.512 11.264q7.168 14.336 12.288 25.088t11.264 16.896q-2.048 9.216-6.144 26.112t-7.68 34.816-6.144 33.792-2.56 24.064q0 5.12 4.608 12.288t10.752 14.848 12.8 12.8 11.776 5.12 12.288-5.12 13.824-11.776 11.264-14.336 4.608-12.8q0-6.144-3.072-20.992t-6.656-33.28-7.68-35.84-7.168-27.648q6.144-5.12 11.776-14.848t13.824-26.112z" p-id="5887"></path></svg>
						
					</text>
					
				</uni-list-item>
				
				<uni-list-item title="设置" showExtraIcon @tap="openHistory">
					<text slot="icon">

						<svg t="1686140889093" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="8084" width="16" height="16"><path d="M512 328c-100.8 0-184 83.2-184 184S411.2 696 512 696 696 612.8 696 512 612.8 328 512 328z m0 320c-75.2 0-136-60.8-136-136s60.8-136 136-136 136 60.8 136 136-60.8 136-136 136z" p-id="8085"></path><path d="M857.6 572.8c-20.8-12.8-33.6-35.2-33.6-60.8s12.8-46.4 33.6-60.8c14.4-9.6 20.8-27.2 16-44.8-8-27.2-19.2-52.8-32-76.8-8-14.4-25.6-24-43.2-19.2-24 4.8-48-1.6-65.6-19.2-17.6-17.6-24-41.6-19.2-65.6 3.2-16-4.8-33.6-19.2-43.2-24-14.4-51.2-24-76.8-32-16-4.8-35.2 1.6-44.8 16-12.8 20.8-35.2 33.6-60.8 33.6s-46.4-12.8-60.8-33.6c-9.6-14.4-27.2-20.8-44.8-16-27.2 8-52.8 19.2-76.8 32-14.4 8-24 25.6-19.2 43.2 4.8 24-1.6 49.6-19.2 65.6-17.6 17.6-41.6 24-65.6 19.2-16-3.2-33.6 4.8-43.2 19.2-14.4 24-24 51.2-32 76.8-4.8 16 1.6 35.2 16 44.8 20.8 12.8 33.6 35.2 33.6 60.8s-12.8 46.4-33.6 60.8c-14.4 9.6-20.8 27.2-16 44.8 8 27.2 19.2 52.8 32 76.8 8 14.4 25.6 22.4 43.2 19.2 24-4.8 49.6 1.6 65.6 19.2 17.6 17.6 24 41.6 19.2 65.6-3.2 16 4.8 33.6 19.2 43.2 24 14.4 51.2 24 76.8 32 16 4.8 35.2-1.6 44.8-16 12.8-20.8 35.2-33.6 60.8-33.6s46.4 12.8 60.8 33.6c8 11.2 20.8 17.6 33.6 17.6 3.2 0 8 0 11.2-1.6 27.2-8 52.8-19.2 76.8-32 14.4-8 24-25.6 19.2-43.2-4.8-24 1.6-49.6 19.2-65.6 17.6-17.6 41.6-24 65.6-19.2 16 3.2 33.6-4.8 43.2-19.2 14.4-24 24-51.2 32-76.8 4.8-17.6-1.6-35.2-16-44.8z m-56 92.8c-38.4-6.4-76.8 6.4-104 33.6-27.2 27.2-40 65.6-33.6 104-17.6 9.6-36.8 17.6-56 24-22.4-30.4-57.6-49.6-97.6-49.6-38.4 0-73.6 17.6-97.6 49.6-19.2-6.4-38.4-14.4-56-24 6.4-38.4-6.4-76.8-33.6-104-27.2-27.2-65.6-40-104-33.6-9.6-17.6-17.6-36.8-24-56 30.4-22.4 49.6-57.6 49.6-97.6 0-38.4-17.6-73.6-49.6-97.6 6.4-19.2 14.4-38.4 24-56 38.4 6.4 76.8-6.4 104-33.6 27.2-27.2 40-65.6 33.6-104 17.6-9.6 36.8-17.6 56-24 22.4 30.4 57.6 49.6 97.6 49.6 38.4 0 73.6-17.6 97.6-49.6 19.2 6.4 38.4 14.4 56 24-6.4 38.4 6.4 76.8 33.6 104 27.2 27.2 65.6 40 104 33.6 9.6 17.6 17.6 36.8 24 56-30.4 22.4-49.6 57.6-49.6 97.6 0 38.4 17.6 73.6 49.6 97.6-6.4 19.2-14.4 38.4-24 56z" p-id="8086"></path></svg>						
					</text>
					
				</uni-list-item>
				<uni-list-item title="关于编创基地" showExtraIcon @tap="openHistory">
					<text slot="icon">
						<svg t="1686140916797" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="9110" width="16" height="16"><path d="M512 832m-64 0a64 64 0 1 0 128 0 64 64 0 1 0-128 0Z" p-id="9111"></path><path d="M790.4 32H233.6C182.72 32 144 78.336 144 132.928v758.144C144 945.664 182.72 992 233.6 992h556.8c50.88 0 89.6-46.336 89.6-100.928V132.928C880 78.336 841.28 32 790.4 32zM233.6 96h556.8c12.736 0 25.6 15.392 25.6 36.928V160h-608v-27.072C208 111.392 220.864 96 233.6 96z m556.8 832H233.6c-12.736 0-25.6-15.392-25.6-36.928V224h608v667.072c0 21.536-12.864 36.928-25.6 36.928z" p-id="9112"></path><path d="M704 560H320a32 32 0 1 0 0 64h384a32 32 0 1 0 0-64zM320 432h256.576a32 32 0 1 0 0-64H320a32 32 0 0 0 0 64z" p-id="9113"></path></svg>
						
					</text>
					
				</uni-list-item>
				</uni-list>
			</view>
		</div>
	</view>
</template>

<script>
	// UNIlist会依赖图标 uni-icons ,会查找 ../uni-icons/uni-icons.vue
	import uniListItem from '@/components/uni-ui/uni-list-item/uni-list-item';

	export default {
		data() {
			return {
				
				userhello:"请登录",
				loginstatus:false
				
				
			}
		},
		methods: {
			openHistory(){
				uni.navigateTo({
					url:"/pages/history/history"
				})
			},
			openIntegral(){
				uni.navigateTo({
					url:"/pages/integral/integral"
				})
			},
			islogin(){
				if(!this.loginstatus){
					uni.navigateTo({
						url:"/pages/login/login"
					})
				}
			}
		},
		onLoad(){
			//TODO:设TOKEN 的cookie 为 token
			if(uni.getStorageSync("usertoken")==""){
				uni.navigateTo({
					url:"/pages/login/login"
				})
			}
			
					
		},
		computed:{
			timehello:{
				get(){
					const timef=new Date().getHours();
					if(timef>8 && timef<12){
						return "早上好";	
					}
					else if(timef>=12 && timef<14){
						return "中午好";
					}else if(timef>=14 && timef<18){
						return "下午好";
					}else {
						return "晚上好";
					}
					
				}
			}
		},
		components:{
			uniListItem,

		}
	
	}
</script>

<style>
	
	/* 引入字体图标 */
	@font-face {font-family: "iconfont";
	  src:url('data:application/x-font-woff2;charset=utf-8;base64,') format('woff2');}
	
	
	* {
		margin: 0;
		padding: 0;
		font-family: "Helvetica Neue",Helvetica,"PingFang SC","Hiragino Sans GB","Microsoft YaHei","微软雅黑",Arial,sans-serif;
	}
 .mine {
	 /* background-color: #f2f3f5; */
	 background-color: white;
 }
.people{
	/* 消除上方空白这个地方使用,padding-top */
	padding-top:50rpx;
	/* position: relative; */
}
.hipo {
	/* position: absolute; */
	/* bottom: 30rpx; */
	font-size:60rpx;
	margin-left: 50rpx;
	font-weight: bold;
	
}
 .peopleimg{
	 width: 150rpx;
	
	 margin-left:50rpx;
	 height: 150rpx;
	 border-radius: 50%;
	 /* 使得文字居中,图片加上这个属性 */
	 vertical-align : middle;
	 
 }
 .peopleabout{
	 margin-top:30rpx;
	 display: flex;
	 width: 750rpx;
	 justify-content: space-around;
 }
 .peopleabout span{
	 /* flex:1; */
 }
 .num{
	 text-align: center;
	 font-size:70rpx;
	 font-weight: bold;
	 
 }
 
 
 .iconfont {
   font-family: "iconfont" !important;
   font-size: 22px;
   font-style: normal;
   -webkit-font-smoothing: antialiased;
   -moz-osx-font-smoothing: grayscale;
 }
 
.icon-liulan:before {
  content: "\e666";
}

.mplist{
	margin-top: 40rpx;
	padding-top:20rpx;
}

.mplist uni-list-item{
	background-color: white;
	border: 0;
}
</style>
